...
pub use guard_platform::config::{
    BrowserDiscovery, BrowserEnrollmentConfig, BrowserProtectionLevel, BrowserSuggestion,
    PolicyConfig, UnsupportedSandboxedBrowser,
};
...
impl EnforcementConfig {
    pub fn policy(&self) -> PolicyConfig {
        PolicyConfig {
            browser_protection_level: self.browser_protection_level,
...
mod tests {
    ...
    fn config() -> EnforcementConfig {
        EnforcementConfig {
            browser_protection_level: BrowserProtectionLevel::Common,
            browsers: Vec::new(),
    ...
    fn unknown_configuration_fields_are_rejected() {
        let error = parse_config(
            br#"{"unexpected_option":true,"browsers":[],"enrolled_exes":[],"ssh_keys":[]}"#,
        )
        .unwrap_err();
    ...
    #[test]
    fn missing_browser_protection_level_defaults_to_common() {
        let config =
            parse_config(br#"{"browsers":[],"enrolled_exes":["/synthetic/exe"],"ssh_keys":[]}"#)
                .unwrap();
        assert_eq!(
