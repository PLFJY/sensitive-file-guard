...
## Step 3 — rename-in / never-opened-before (R1: SETTLED case closed, zero-settle attacker path RE-OPENED)
...
- **KERNEL 7.1 MEASURED REALITY (definitive C probes, `fid-definitive.c`/`fid-sep.c`)**: a move event's fid is the MARKED/PARENT DIRECTORY's handle — NEVER the moved file's; back-to-back move-in+move-out coalesce into one event with mask `FAN_MOVE` (0xc0); `FAN_REPORT_DFID_NAME` (0xC00 = DIR_FID|NAME; note 0x400 alone is DIR_FID *without* the name) adds the moved object's NAME. The moved file's own handle is NOT in the events — the earlier "MOVED_TO fid = file" C-probe conclusion was wrong (invalidated by the loop-harness bugs). The learner therefore records each marked dir's handle and resolves `parent/name` (DFID_NAME) to the moved file's handle while it remains there.
...
- **Zero-settle acceptance (live, split by kernel capability)** — `scripts/linux/test-step3-zero-settle-root.sh` (isolated loop ext4):
  - SETTLED case (moved-in object that REMAINS, later renamed out + immediate open): **LIVE PASS — 1000/1000 denied, 0 recovery, no settle between rename-out and open** (DFID_NAME resolution learns the file's handle).
  - Runtime-created subdirectory (marked by the learner's 2 s refresh; settled attack inside it): **LIVE PASS — 200/200 denied, 0 recovery**.
  - FAST attack (rename-in → immediately rename-out → immediate open, 10k): **MEASURED — 145/10000 recovered (~1.45%)**. The kernel provides no moved-file identity in the events (fid=parent dir), so resolution races the immediate rename-out; this fraction is kernel-limited, not enforceable by userspace on this kernel. F3 remains RE-OPENED for the fast attack pending a kernel-capable mechanism (e.g. LSM); the ordering + settled + subdir guarantees are live-verified.
...
| never-opened-before rename-in gap (ZERO-SETTLE attacker path) | RE-OPENED (R1) — pending live | `test-step3-zero-settle-root.sh` (10k rename-in→immediate rename-out→immediate open, 0 recovery; runtime-subdir refresh) | F3 cannot be CLOSED until this passes |
...
## Final phase verdict
`SETTLED case: PASS (snapshot + topology, live 8/8). Zero-settle attacker path: RE-OPENED (R1) — cross-group ordering implemented (synchronous drain + shared mutex), live 10k zero-settle stress pending in test-step3-zero-settle-root.sh; F3 CLOSED only when that passes with 0 recovery`
