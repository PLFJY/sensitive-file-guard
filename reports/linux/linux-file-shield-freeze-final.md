...
| F3 | LFH2 Step 3 topology group for never-opened-before rename-in | RE-OPENED (R1); ordering + settled + subdir LIVE PASS; fast attack KERNEL-LIMITED | cross-group ordering implemented (permission hot path synchronously drains causally-prior topology events under the learner's mutex); kernel 7.1 measured: move fid = PARENT DIR (never the file), DFID_NAME = parent+name → learner records marked-dir handles and resolves `parent/name`. LIVE: settled 1000/1000 + subdir 200/200 denied (0 recovery); fast attack (in→immediate out→immediate open, 10k) measured 145/10000 recovered (~1.45%) — the kernel provides no moved-file identity, so the as-prescribed 0-recovery fast-attack gate is NOT implementable via fanotify on this kernel (evidence: live-host-step3-fin-20260820-005538, fid-definitive/fid-sep probes) |
